(function(){
  ...
  function randFloat(rng, min = 0, max = 1) { return min + (max - min) * rng(); }
  function randInt(rng, min, maxInclusive) { const x = rng(); return Math.floor(min + (maxInclusive - min + 1) * x); }
  ...
  const GEO_TYPES = ['Rocky','Iron-Core','Silicate','Frozen','Metallic','Carbon-Based','Sulfide','Basaltic','Granitic','Sedimentary','Volcanic','Crystalline','Oceanic','Desert','Tectonic','Metamorphic','Calcareous','Ferrous','Regolithic','Chondritic'];
  const ATMOS = ['None','Thin','Nitrogen-Oxygen','Carbon-Dioxide','Methane','Ammonia','Hydrogen','Helium','Hydrogen-Sulfide','Argon','Neon','Chlorine','Fluorine','Sulfur-Dioxide','Nitrogen','Oxygen-Rich','Toxic','Corrosive','Inert','Reducing'];
  const FAUNA = ['None','Microbial','Arthropod','Reptilian','Mammalian','Sentient','Mechanical','Energy-Based','Avian','Amphibian','Cephalopod','Insectoid','Crystalline','Fungoid','Aquatic','Silicon-Based','Hive-Mind','Parasitic','Symbiotic','Swarm'];
  const FLORA = ['None','Unicellular-Algae','Moss','Fern','Gymnosperm','Angiosperm','Fungal','Crystalline-Life','Lichen','Conifer','Succulent','Carnivorous','Bioluminescent','Symbiotic','Photosynthetic','Chemosynthetic','Spore-Based','Vine','Tree','Aquatic-Plants'];
  function romanize(num){ if(num<=0) return ''; const map=[[1000,'M'],[900,'CM'],[500,'D'],[400,'CD'],[100,'C'],[90,'XC'],[50,'L'],[40,'XL'],[10,'X'],[9,'IX'],[5,'V'],[4,'IV'],[1,'I']]; let n=Math.floor(num),out=''; for(const [v,s] of map){ while(n>=v){out+=s;n-=v;} } return out; }
  function pickSystemName(rng){ return `${choice(rng, STAR_NAME_PREFIX)} ${choice(rng, STAR_NAME_SUFFIX)}`; }
  function pickSystemCode(rng){ const A=65; const c1=String.fromCharCode(A+randInt(rng,0,25)); const c2=String.fromCharCode(A+randInt(rng,0,25)); const num=String(randInt(rng,0,9999)).padStart(4,'0'); return `${c1}${c2}-${num}`; }
  function pickPlanetName(rng, used){ for(let t=0;t<100;t++){ const base=choice(rng, PLANET_NAMES_BASE); const withRoman = (rng()<0.25)? `${base} ${romanize(randInt(rng,1,12))}`: base; if(!used.has(withRoman)){ used.add(withRoman); return withRoman; } } let i=1; for(;;){ const name=`${choice(rng, PLANET_NAMES_BASE)}-${i++}`; if(!used.has(name)){ used.add(name); return name; } } }
  function pickMaybeCombo(rng, arr){ if(rng()<0.18){ const a=choice(rng,arr); let b=choice(rng,arr); if(b===a) b=choice(rng,arr); return `${a}+${b}`; } return choice(rng,arr); }
  function pickGeo(rng){ return pickMaybeCombo(rng,GEO_TYPES);} 
  function pickAtmo(rng){ return pickMaybeCombo(rng,ATMOS);} 
  function pickFauna(rng){ return pickMaybeCombo(rng,FAUNA);} 
  function pickFlora(rng){ return pickMaybeCombo(rng,FLORA);} 

  // ===== physics.js =====
  function orbitalSpeed(massCentralKg, radiusM){ return Math.sqrt(G * massCentralKg / radiusM); }
  function orbitalPeriod(radiusM, speedMps){ return (TAU * radiusM) / speedMps; }
  function computeTimeScale(outerPeriodSec, targetMinSec=120, targetMaxSec=300){ const target = clamp(targetMinSec + (targetMaxSec - targetMinSec) * 0.5, targetMinSec, targetMaxSec); let scale = outerPeriodSec / target; scale = clamp(scale, 500, 2000); return { timeScale: scale, targetPeriodSec: target }; }

  // ===== generator: system.js =====
  function classifyStar(massKg){ if (massKg < 1e29) return 'D'; if (massKg < 2e29) return 'C'; if (massKg < 3e29) return 'B'; if (massKg < 4e29) return 'A'; return 'S'; }
  function pickOrbits(rng, starRadiusM, planetCount){ const minR = starRadiusM * 3, maxR = starRadiusM * 30; const eps=1e-6; const a0 = minR * (1+eps); const orbits = new Array(planetCount); orbits[0] = a0; for(let i=1;i<planetCount;i++){ const k = randFloat(rng, 1.1, 2.0); let candidate = orbits[i - 1] * k; if (candidate > maxR * 0.95) { candidate = maxR * 0.95; } orbits[i] = candidate; if (candidate >= maxR * 0.95) { return orbits.slice(0, i + 1); } } return orbits; }
  function computeHillRadius(aM, mP, mS){ return aM * 0.01 * Math.cbrt(mP / mS); }
  function pickPlanetPhysicals(rng, starMassKg){ const fMin=1e-6, fMax=1e-3; for(let tries=0; tries<50; tries++){ const radiusKm = randInt(rng, 1000, 20000); const rM = radiusKm * KM; const volume = (4/3) * Math.PI * rM * rM * rM; const rhoLow = Math.max(3000, (starMassKg * fMin) / volume); const rhoHigh = Math.min(8000, (starMassKg * fMax) / volume); if (rhoLow <= rhoHigh){ const density = randFloat(rng, rhoLow, rhoHigh); const mass = density * volume; return { radiusKm, radiusM: rM, densityKgPerM3: density, massKg: mass }; } } const radiusKm = 8000; const rM = radiusKm * KM; const volume = (4/3) * Math.PI * rM * rM * rM; const density = clamp((starMassKg * 1e-5) / volume, 3000, 8000); const mass = density * volume; return { radiusKm, radiusM: rM, densityKgPerM3: density, massKg: mass };
  }
  function generateSolarSystem(rng){ const starRadiusKm = randInt(rng, 30000, 70000); const starRadiusM = starRadiusKm * KM; const starMassKg = randFloat(rng, 1e28, 5e29); const starClass = classifyStar(starMassKg); const systemName = pickSystemName(rng); const systemCode = pickSystemCode(rng); const planetCount = randInt(rng, 1, 10); const orbits = pickOrbits(rng, starRadiusM, planetCount); const usedNames = new Set(); const planets = []; for(let i=0;i<orbits.length;i++){ const aM = orbits[i]; const phys = pickPlanetPhysicals(rng, starMassKg); const v = orbitalSpeed(starMassKg, aM); const T = orbitalPeriod(aM, v); const hillM = computeHillRadius(aM, phys.massKg, starMassKg); const hue=randInt(rng,0,360); const sat=randInt(rng,40,90); const light=randInt(rng,40,70); const color=`hsl(${hue}, ${sat}%, ${light}%)`; planets.push({ name: pickPlanetName(rng, usedNames), geo: pickGeo(rng), atmo: pickAtmo(rng), fauna: pickFauna(rng), flora: pickFlora(rng), color: color, massKg: phys.massKg, densityKgPerM3: phys.densityKgPerM3, radiusKm: phys.radiusKm, radiusM: phys.radiusM, orbitRadiusM: aM, orbitRadiusKm: aM / KM, orbitSpeedMps: v, orbitSpeedKmps: v / 1000, orbitPeriodSec: T, phaseRad: randFloat(rng, 0, Math.PI*2), hillRadiusM: hillM, hillRadiusKm: hillM / KM }); } for(let i=1;i<planets.length;i++){ const prev = planets[i-1], cur = planets[i]; const radialGap = cur.orbitRadiusM - prev.orbitRadiusM; if (radialGap <= prev.hillRadiusM + cur.hillRadiusM){ const needed = prev.hillRadiusM + cur.hillRadiusM + 1; const newA = prev.orbitRadiusM + needed; cur.orbitRadiusM = newA; cur.orbitRadiusKm = newA / KM; const v = orbitalSpeed(starMassKg, newA); cur.orbitSpeedMps = v; cur.orbitSpeedKmps = v / 1000; cur.orbitPeriodSec = orbitalPeriod(newA, v); cur.hillRadiusM = computeHillRadius(newA, cur.massKg, starMassKg); cur.hillRadiusKm = cur.hillRadiusM / KM; } } const outer = planets[planets.length - 1]; const time = computeTimeScale(outer.orbitPeriodSec, 120, 300); const systemRadiusM = starRadiusM * 30; return { name: systemName, code: systemCode, star: { class: starClass, massKg: starMassKg, radiusKm: starRadiusKm, radiusM: starRadiusM }, systemRadiusM, planets, time };
  }
  ...
  function setupCanvases({ bgId = 'bgCanvas', mainId = 'mainCanvas', onResize = null } = {}) {
    ...
    const handle = () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(applySize, 100); };
  ...
  // ===== starfield.js =====
  function generateStarfield(rng, width, height) {
  ...
  function main() {
    ...
    if (resetBtn) { resetBtn.addEventListener('click', () => { controls.resetView(fitRadius); }); }
    ...
    const dt = 1/30;
    const timeScale = system.time.timeScale;
    let gameTime = 0;
    ...
    function frame() {
      ...
      for(const p of system.planets){ drawOrbit(ctx, camera, p.orbitRadiusM); }
      for(const p of system.planets){ const angle=p.phaseRad+(p.orbitSpeedMps/p.orbitRadiusM)*gameTime; const wx=p.orbitRadiusM*Math.cos(angle); const wy=p.orbitRadiusM*Math.sin(angle); drawHillSphere(ctx,camera,wx,wy,p.hillRadiusM); }
      drawSun(ctx, camera, system.star, rotInner, rotOuter);
      for(const p of system.planets){ const angle=p.phaseRad+(p.orbitSpeedMps/p.orbitRadiusM)*gameTime; const wx=p.orbitRadiusM*Math.cos(angle); const wy=p.orbitRadiusM*Math.sin(angle); drawPlanet(ctx,camera,wx,wy,p.radiusM,p.color); }

      requestAnimationFrame(frame);
    }
    requestAnimationFrame(frame);
    window.__SOLAR__ = { seed, rng, canvases, system, camera, hud, controls };
  }
